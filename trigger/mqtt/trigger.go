...
func (t *Trigger) getHanlder(handler *clientHandler, parsed Topic) func(mqtt.Client, mqtt.Message) {
	return func(client mqtt.Client, msg mqtt.Message) {
		...
		result, err := runHandler(handler, payload, topic, params)
		if err != nil {
...
func runHandler(handler *clientHandler, payload, topic string, params map[string]string) (map[string]interface{}, error) {
	out := &Output{
		Message:     payload,
		Topic:       topic,
		TopicParams: params,
		AssetName:   handler.settings.AssetName,
		SensorName:  handler.settings.SensorName,
		SensorType:  handler.settings.SensorType,
	}
